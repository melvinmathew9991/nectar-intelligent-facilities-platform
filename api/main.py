"""
Nectar Predictive Maintenance & Connectivity API (Bonus)
=========================================================
Two bonus deliverables from the brief, in one FastAPI app:

1. Model Deployment -- POST /predict_failure accepts RAW telemetry (an
   asset_id to score against its recent history, or an explicit telemetry
   window). Feature engineering runs inside the endpoint via the exact same
   features.py module used in training (train/serve parity), not a
   pre-engineered 82-feature dict.
2. GraphQL -- a real GraphQL schema (Strawberry) mounted at /graphql,
   implementing the brief's example queries (assets connected to Chiller-01,
   downstream impact of an asset failure, assets under a site, isolated
   assets) on top of Task 5's graph.py.
...
@app.get("/")
def root():
    return {"service": "Nectar Intelligent Facilities API",
            "endpoints": ["/predict_failure", "/health",
                          "/assets/{asset_id}/downstream_impact",
                          "/assets/{asset_id}/upstream_dependencies",
                          "/graphql", "/docs"]}
